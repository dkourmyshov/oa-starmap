...
class OASystemFile(BaseModel):
    ...
    hide_designation_only: bool = False
    """Hide add-on stars that carry nothing but a catalogue designation.

    The same intent as the rule above, for entries whose comment is empty so no
    substring can catch them. An entry qualifies only when it has no world bound
    to it, no system name, no comment, and a name that is a designation and not
    a name — `alottafictionalstars.stc` is the file this was written for, 29
    G2V stars with every other field blank.

    All four conditions are required, which is what makes it safe: Enigma,
    Geminga, Arkab Prior B and the Arkab Prior Necklace are equally uncommented
    and stay, because somebody named them. So does anything a world is later
    bound to, without this list being touched.
    """
