...
A note on method, because the first pass got St Xye right for the wrong reason. It was refused on the ground that its gate partner lay nine degrees away, inside the band where this add-on seems to place systems it has no star for along the line of their link. That is a real pattern but a weak test, and this file already holds the strong one: the IAU constellation cones, with a radius per constellation. Testing a proposed direction against the constellation the article states is cheap, decisive either way, and should come first.
